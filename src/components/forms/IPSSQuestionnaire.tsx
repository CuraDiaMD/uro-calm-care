...
export function IPSSQuestionnaire({ onComplete }: IPSSQuestionnaireProps) {
  ...
  const getOptionsForQuestion = (index: number) => {
    if (index === 6) return t.ipss.optionsNocturia.map((label, i) => ({ score: i, label }));
    if (index === 7) return t.ipss.optionsQoL.map((label, i) => ({ score: i, label }));
    return t.ipss.optionsStandard.map((label, i) => ({ score: i, label }));
  };
  ...
  const handleSubmit = () => {
    ...
    toast.success(t.ipss.completed);
    onComplete();
  };
  
  const question = t.ipss.questions[currentQuestion];
  const options = getOptionsForQuestion(currentQuestion);
  
  return (
    <div className="screen-container">
      <QuestionnaireProgress current={currentQuestion + 1} total={8} category={question.category} />
      ...
      <QuestionnaireNav
        currentQuestion={currentQuestion} totalQuestions={8}
